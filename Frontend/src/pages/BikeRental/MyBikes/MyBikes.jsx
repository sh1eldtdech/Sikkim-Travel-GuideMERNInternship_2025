...
const MyBikes = () => {
  ...
  const handleToggleStatus = async (bike) => {
    ...
    try {
      ...
      await API.patch(`/bikes/${bike._id}/status`, { isActive: newStatus });
      setBikes((prev) =>
        prev.map((b) =>
          b._id === bike._id ? { ...b, isActive: newStatus } : b,
        ),
      );
      toast.success(`Bike ${newStatus ? "activated" : "deactivated"}!`);
    } catch (err) {
      console.error("[MyBikes] error details:", err);
      console.error("[MyBikes] status update failed", {
        endpoint: `/bikes/${bike._id}/status`,
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl p-12 text-center shadow-sm border border-gray-100 flex flex-col items-center">
          ...
          <p className="text-gray-500 font-medium mb-6">
            Start capitalizing on your fleet by adding your first bike.
          </p>
          <button
            onClick={() => navigate("/owner/bike-rental/add-bike")}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {bikes.map((bike) => (
            <div
              key={bike._id}
              className={`bg-white rounded-3xl overflow-hidden shadow-sm border transition-shadow hover:shadow-lg ${!bike.isActive ? "border-gray-100 opacity-80" : "border-gray-200"}`}
            >
              {/* Image Section */}
              <div className="relative h-48 bg-gray-100">
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                  {!bike.isApproved ? (
                    ...
                    <span className="bg-gray-100 text-gray-600 px-3 py-1 text-xs font-bold rounded-full shadow-sm">
                      Inactive
                    </span>
                  )}
                </div>
              ...
              <div className="p-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    onClick={() =>
                      navigate(`/owner/bike-rental/edit-bike/${bike._id}`, {
                        state: { bikeData: bike },
                      })
                    }

...
const inputCls =
  "w-full px-4 py-3 rounded-xl border border-gray-200 bg-white shadow-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-200 outline-none transition-all text-sm font-medium text-gray-800 placeholder-gray-400";
...
const AddBike = () => {
  ...
  const isEditMode = Boolean(id || editData?._id);
  ...
  // newImages: File objects selected by user in this session
  const [newImages, setNewImages] = useState([]);
  const [loading, setLoading] = useState(false);
  const [pageLoading, setPageLoading] = useState(Boolean(id && !editData));
  ...
  useEffect(() => {
    ...
    const fetchBikeForEdit = async () => {
      setPageLoading(true);
      setPageError("");
      try {
        const { data } = await API.get(`/bikes/owner/my-bikes/${id}`);
        ...
        setPageError(
          err.response?.data?.message || "Failed to load this bike listing.",
        );
      } finally {
  ...
  const handleImages = (e) => {
    ...
    // Total cap: 4 images across existing + new
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        await API.put(`/bikes/${id || editData._id}`, fd, {
          headers: authHeaders,
        });
      ...
      toast.error(
        err.response?.data?.message || "Failed to save bike. Try again.",
      );
    } finally {
      setLoading(false);
    }
  ...
  if (pageLoading) {
    return (
      <PageContainer size="narrow" bottom="normal">
    ...
    return (
      <PageContainer size="narrow" bottom="normal">
        <div className="bg-white rounded-3xl p-10 shadow-sm border border-gray-100 text-center">
          ...
          <p className="text-gray-500 font-medium mb-6">{pageError}</p>
          <button
            onClick={() => navigate("/owner/bike-rental/my-bikes")}
            className="px-6 py-3 rounded-xl bg-orange-500 text-white font-bold hover:bg-orange-600 transition-colors"
          >
            Back to My Bikes
          </button>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">
              {isEditMode ? "Edit Listing" : "List a New Bike"}
            </h1>
            <p className="text-sm font-medium text-gray-500">
              Provide details about the rental
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Field
              label="Full Bike Name"
              required
              desc="Include make and model"
            >
              <input
                name="name"
          ...
          <div className="mt-6">
            <Field
              label="Description"
              required
              desc="Highlight the condition and special traits"
            >
              <textarea
                name="description"
                rows={4}
          ...
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium mt-0.5">
                Select all that apply to this rental
              </p>
            </div>
          </div>
            ...
            <div className="text-center">
              <p className="text-sm font-bold text-gray-700 group-hover:text-orange-600">
                {newImages.length > 0
                  ...
                  ? `${existingImages.length} saved · click to add more`
          ...
          {(existingImages.length > 0 || newImages.length > 0) && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
              {existingImages.map((src, i) => (
                <div
                  key={`ex-${i}`}
                  className="relative group rounded-2xl overflow-hidden aspect-square bg-gray-100 border border-gray-200"
                >
                  <img src={src} alt="saved" className="w-full h-full object-cover" />
                  <span className="absolute bottom-1 left-1 text-[10px] bg-black/40 text-white px-1.5 py-0.5 rounded font-semibold">Saved</span>
                  <button
                    type="button"
              ...
              {newImages.map((file, i) => (
                <div
                  key={`new-${i}`}
                  className="relative group rounded-2xl overflow-hidden aspect-square bg-gray-100 border border-blue-200"
                >
                  <img src={URL.createObjectURL(file)} alt="new" className="w-full h-full object-cover" />
                  <span className="absolute bottom-1 left-1 text-[10px] bg-blue-500/80 text-white px-1.5 py-0.5 rounded font-semibold">New</span>
                  <button
                    type="button"
                    onClick={() => removeNew(i)}
                    className="absolute top-2 right-2 w-8 h-8 rounded-full bg-white/90 shadow-sm text-red-500 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-50"

  ...
  try {
    ...
    const hotelsWithRooms = await Promise.all(
      hotels.map(async (hotel) => {
        ...
        const startingPrice =
          rooms.length > 0
            ? Math.min(...rooms.map((r) => r.minPrice || r.price || 0))
            : 0;

        const highestPrice =
          rooms.length > 0
            ? Math.max(...rooms.map((r) => r.maxPrice || r.price || 0))
            : 0;
    ...
    res.status(500).json({
      message: "An error occurred while fetching hotels. Please try again.",
    });
  }
});

// OWNER ROUTES — Protected, only approved owners
// GET /hotels/owner/my-hotels (MUST come before GET /:id)
...
router.post(
  ...
  async (req, res) => {
    try {
      const { name, district, location, description, amenities, policies } =
        req.body;

      if (!name || !district || !location || !description) {
        return res.status(400).json({
          message: "Name, district, location and description are required",
        });
      }
      ...
      res.status(500).json({
        message: "An error occurred while adding the hotel. Please try again.",
      });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (hotel.owner.toString() !== req.owner._id.toString()) {
        return res
          .status(403)
          .json({ message: "Not authorized to edit this hotel" });
      }
      ...
      if (req.files && req.files.length > 0) {
        const newImages = req.files.map((f) => f.path || f.secure_url || f.url);
...
router.patch("/:id/status", protectOwner, async (req, res) => {
  try {
    ...
    res.json({
      message: `Hotel ${hotel.isActive ? "reactivated" : "deactivated"} successfully`,
      hotel,
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
...
// GET /hotels/:id (MUST come after all specific routes)
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!hotel || !hotel.isActive || !hotel.isApproved) {
      return res
        .status(404)
        .json({ message: "Hotel not found or pending approval" });
    }
